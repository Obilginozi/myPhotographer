...
/**
 * REST controller for managing {@link com.fotografcim.domain.File}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class FileResource {
    ...
    public String uploadProfilPhoto(@RequestParam("file") MultipartFile file) throws Exception {
        String test = file.getOriginalFilename().toString();
        log.info(test);
        return test;
    }
     ...
     */
    @PutMapping("/files/{id}")
    public ResponseEntity<File> updateFile(@PathVariable(value = "id", required = false) final UUID id, @RequestBody File file)
    ...
    public ResponseEntity<File> partialUpdateFile(@PathVariable(value = "id", required = false) final UUID id, @RequestBody File file)
        throws URISyntaxException {
        ...
        Optional<File> result = fileRepository
            ...
            .map(existingFile -> {
                ...
                if (file.getPhoto() != null) {
                    existingFile.setPhoto(file.getPhoto());
                }
                if (file.getPhotoContentType() != null) {
                    existingFile.setPhotoContentType(file.getPhotoContentType());
                }
