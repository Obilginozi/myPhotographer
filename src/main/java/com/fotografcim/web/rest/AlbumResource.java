package com.fotografcim.web.rest;
...
import com.fotografcim.repository.FileRepository;
import com.fotografcim.service.MailService;
import com.fotografcim.service.UserService;
import com.fotografcim.web.rest.errors.BadRequestAlertException;
...
/**
 * REST controller for managing {@link com.fotografcim.domain.Album}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class AlbumResource {
    ...
    private static final File fileEmp = new File();
    private static final List<Boolean> fileList = new ArrayList<>();
    ...
    public AlbumResource(FileRepository fileRepository, AlbumRepository albumRepository, UserService userService, MailService mailService) {
    ...
    @PostMapping("/albums")
    public ResponseEntity<Album> createAlbum(@RequestPart Album album, @RequestPart List<MultipartFile> files)
        ...
        String token = mailService.callToken(album.getClient().getClientMail());
        ...
        for (MultipartFile file : files) {
            ...
            file1.setAlbum(result);
            file1.setName(
                user.getId().toString() +
                "---" +
                album.getClient().getId().toString() +
                "---" +
                result.getId().toString() +
                "---" +
                formatter.format(date)
            );
            file1.setPhoto(bytes);
            file1.setPhotoContentType(file.getContentType());
            File result1 = fileRepository.save(file1);
        ...
        if (!Arrays.asList(fileList).contains(false)) {
            mailService.sendAlbumToken(user, result);
        }
    ...
    public ResponseEntity<Album> updateAlbum(@PathVariable(value = "id", required = false) final UUID id, @RequestBody Album album)
    ...
    public ResponseEntity<Album> partialUpdateAlbum(@PathVariable(value = "id", required = false) final UUID id, @RequestBody Album album)
        throws URISyntaxException {
        ...
        Optional<Album> result = albumRepository
            ...
            .map(existingAlbum -> {
                if (album.getToken() != null) {
                    existingAlbum.setToken(album.getToken());
                }
                if (album.getCreateDate() != null) {
                    existingAlbum.setCreateDate(album.getCreateDate());
                }
    ...
    /**
     * {@code GET  /albums/:token} : get the "token" album.
     *
     * @param token the id of the album to retrieve.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the album, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/albums/{token}")
    public ResponseEntity<Album> getAlbum(@PathVariable String token) {
        log.debug("REST request to get Album : {}", token);
        Optional<Album> album = albumRepository.findAlbumByToken(token);
