...
export const AlbumDetail = (props: RouteComponentProps<{ token: string }>) => {
    ...
    const tempImages = test.id
      ? test.files.map(file => {
          return (
            <img
              key={file.id}
              src={'data:image/png;base64,' + file.photo}
              style={{ alignItems: 'flex-start', maxWidth: '100%', marginTop: '10px' }}
            />
          );
        })
      : null;
    ...
    <Row>
      <Col md="12">
        ...
        <dl className="jh-entity-details">
          ...
          <dd>{albumEntity.client ? albumEntity.client.clientName + ' ' + albumEntity.client.clientSurname : ''}</dd>
          <div>{images}</div>
        </dl>
      </Col>
    </Row>
